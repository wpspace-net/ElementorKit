 ...
 *
 * @package ElementorKit/Template_Kit_Import
 * @since 1.0.0
 */
...
class Importer extends Base {
	...
	public function handle_template_import( $template_kit_id, $template_index ) {
		...
		$template_kit = template_kit_import_get_builder( $template_kit_id );
	...
	public function install_template_kit_zip_to_db( $temporary_zip_file ) {
		...
		// Check and modify Elementor Pro plugin information
		if ( ! empty( $manifest_data['required_plugins'] ) && is_array( $manifest_data['required_plugins'] ) ) {
			foreach ( $manifest_data['required_plugins'] as $key => $plugin ) {
				if ( isset( $plugin['name'] ) && $plugin['name'] === 'Elementor Pro' ) {
					$manifest_data['required_plugins'][ $key ]['name'] = 'PRO Elements';
					$manifest_data['required_plugins'][ $key ]['file'] = 'pro-elements/pro-elements.php';
					$manifest_data['required_plugins'][ $key ]['author'] = 'PRO Elements team';
				}
			}
		}
		...
		if ( $manifest_data && ! empty( $manifest_data['version'] ) ) {
			...
			$page_builder = TEMPLATE_KIT_IMPORT_TYPE_ELEMENTOR;
		}
		...
		if ( $post_id && ! is_wp_error( $post_id ) ) {
			...
			update_post_meta( $post_id, 'tk_manifest', $manifest_data );
			update_post_meta( $post_id, 'tk_folder_name', $template_kit_random_folder );
			update_post_meta( $post_id, 'tk_builder', $page_builder );
			...
			update_post_meta( $post_id, 'tk_source_zip_url', $template_kit_extract_url . '/' . $source_file_name );
