...
class Builder_Elementor extends Builder {
	...
	private function import_json_file_to_elementor_library( $json_file_path, $template_index ) {
		...
		if ( $result[0] && $result[0]['template_id'] ) {
			...
			update_post_meta( $imported_template_id, 'tk_source_kit', $this->kit_id );
			update_post_meta( $imported_template_id, 'tk_source_index', $template_index );
	...
	public function check_if_imported_template_is_getting_deleted( $post_to_be_deleted ) {
		if ( class_exists( '\Elementor\Plugin' ) ) {
			...
			if ( $post && 'elementor_library' === $post->post_type ) {
				$source_kit_id = get_post_meta( $post->ID, 'tk_source_kit', true );
				if ( $source_kit_id ) {
					$source_kit_index = get_post_meta( $post->ID, 'tk_source_index', true );
					try {
						...
						update_post_meta( $this->kit_id, 'tk_manifest', $manifest );
					} catch ( \Exception $e ) {
						// noop
					}
				}
	...
	public function get_list_of_unmet_requirements( $template ) {
		...
		if ( ! empty( $template['elementor_pro_required'] ) && ! class_exists( '\ElementorPro\Plugin' ) ) {
			$unmet_requirements[] = __( 'PRO Elements is required for this Template.', 'template-kit-export' );
		}
