...
class Settings extends API {
	...
	public function get_server_limits() {
		...
		try {
			$response = wp_safe_remote_get( 'https://api.elementorkit.site', [
			...
			if ( $response && ! is_wp_error( $response ) && $response_code === 200 ) {
				...
				$limits['plugin_api'] = [
					'ok'      => false,
					'title'   => 'Plugin API',
					'message' => "Plugin API failed. Status '$response_code'. Please ensure PHP is allowed to connect to the host 'api.elementorkit.site' - or ask your hosting provider to do this if you’re unsure. " . ( is_wp_error( $response ) ? $response->get_error_message() : '' ),
				];
			}
		} catch ( \Exception $e ) {
			$limits['plugin_api'] = [
				'ok'      => true,
				'title'   => 'Plugin API',
				'message' => "Plugin API failed. Please contact the hosting provider and ensure PHP is allowed to connect to the host 'api.elementorkit.site'. " . $e->getMessage(),
			];
		}
		...
		$pro_elements_installed = class_exists( '\ElementorPro\Plugin' );
		$limits['pro_elements'] = [
			'ok'      => $pro_elements_installed,
			'title'   => 'PRO Elements',
			'message' => $pro_elements_installed ? 'PRO Elements is installed and active.' : 'Some of our Premium Templates will fail to install without PRO Elements.',
		];
