<?php
/**
 * ElementorKit: Template Kits Import
 *
...
class Requirements extends API {
	...
	public function install_requirement( $request ) {
		...
		if ( ! empty( $requirement ) && ! empty( $requirement['plugin'] ) ) {
			...
			// Handle PRO Elements installation from GitHub zip
			if ( $plugin_slug === 'pro-elements' ) {
				$install_status = $this->install_plugin_from_zip( 'https://github.com/proelements/proelements/releases/latest/download/pro-elements.zip', 'pro-elements/pro-elements.php' );
				if ( ! $install_status['success'] ) {
					return $this->format_error(
						'installRequirement',
						'install_failed',
						'PRO Elements install failed: ' . $install_status['errorMessage']
					);
				}
	...
	private function install_plugin_from_zip( $zip_url, $plugin_file ) {
		...
		if ( $activate_status && ! is_wp_error( $activate_status ) ) {
			...
			$status['errorMessage'] = 'Plugin installed but could not be activated';
		}
