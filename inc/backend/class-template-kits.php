<?php
/**
 * ElementorKit: Template Kit integration
 *
 * Template Kit.
 *
...
class Template_Kits extends Base {
	...
	public function before_delete_post( $post_to_be_deleted ) {
		...
		if ( $post && 'elementor_library' === $post->post_type ) {
			...
			\Template_Kit_Import\Builder_Elementor::get_instance()->check_if_imported_template_is_getting_deleted( $post_to_be_deleted );
		} elseif ( $post && 'tk_import' === $post->post_type ) {
	...
	private function load_template_kit_library() {
		require_once ELEMENTORKIT_DIR . 'vendor/template-kit-import/template-kit-import.php';
	}
	...
	public function process_zip_file( $zip_file ) {
		...
		return \Template_Kit_Import\Importer::get_instance()->install_template_kit_zip_to_db( $zip_file );
	}
	...
	public function delete_template_kit( $template_kit_id ) {
		...
		\Template_Kit_Import\Delete::get_instance()->delete_template_kit( $template_kit_id );
	...
	public function get_installed_template_kits() {
		$this->load_template_kit_library();
		// Reach into our included Template Kit import plugin functions to do the actual import.
		$uploaded_kits  = \Template_Kit_Import\CPT_Kits::get_instance()->get_all_uploaded_kits();
		$installed_kits = [];
		try {
			foreach ( $uploaded_kits as $template_kit ) {
				...
				$template_kit_manager = \Template_Kit_Import\template_kit_import_get_builder( $template_kit_id );
	...
	public function get_installed_template_kit( $template_kit_id ) {
		...
		$template_kit = \Template_Kit_Import\template_kit_import_get_builder( $template_kit_id );
		if ( ! $template_kit ) {
	...
	public function import_single_template( $template_kit_id, $template_id, $import_again ) {
		...
		$imported_template_id = \Template_Kit_Import\Importer::get_instance()->handle_template_import( $template_kit_id, $template_id );
